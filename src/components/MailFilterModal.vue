...
export default {
	...
	methods: {
		...
		updateAction(action) {
			const index = this.clone.actions.findIndex((items) => items.id === action.id)
			this.$set(this.clone.actions, index, action)
		},
		deleteAction(action) {
			this.clone.actions = this.clone.actions.filter((items) => items.id !== action.id)
		},
		async storeFilter() {
			this.$emit('store-filter', structuredClone(this.clone))
		},
		closeModal() {
			this.$emit('close')
		},
